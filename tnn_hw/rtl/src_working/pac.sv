...
module pac (out, in, aclk, grst);
    ...
    localparam STAGES = $clog2(INPUT_SIZE)-1;
    localparam OUT_RES = $clog2(INPUT_SIZE);
    localparam NUM = 2*INPUT_SIZE - OUT_RES-2;
    ...
    generate
        ...
        begin: stages
            ...
            for (j = 0; j < INPUT_SIZE/(1<<(i+2)); j = j + 1)
            begin: adders
                
                adder #(i+1) a1 (.out(temp[(INPUT_SIZE>>(i+1))*((1<<(i+2))-i-3) + j*(i+2) : (INPUT_SIZE>>(i+1))*((1<<(i+2))-i-3) + j*(i+2) + (i+1)]),
                                 .a(temp[(INPUT_SIZE>>i)*((1<<(i+1))-i-2) + 2*j*(i+1) : (INPUT_SIZE>>i)*((1<<(i+1))-i-2) + 2*j*(i+1) + i]),
                                 .b(temp[(INPUT_SIZE>>i)*((1<<(i+1))-i-2) + (2*j+1)*(i+1) : (INPUT_SIZE>>i)*((1<<(i+1))-i-2) + (2*j+1)*(i+1) + i]),
                                 .cin(in[INPUT_SIZE/2 + (INPUT_SIZE>>(i+1))*((1<<i)-1) + j])
                                );
