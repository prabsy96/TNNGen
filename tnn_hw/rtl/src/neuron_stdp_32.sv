...
module neuron_stdp (output_spike, weight_update_en, input_spikes, capture, minus, search, backoff, min, F, aclk, gclk, rst);
    ...
    input logic [0:P-1] capture;
    input logic [0:P-1] minus;
    input logic [0:P-1] search;
    input logic [0:P-1] backoff;
    input logic [0:P-1] min;
    input logic [0:5] F;
    input logic weight_update_en;
    input logic  aclk, gclk, rst;
    output logic  output_spike;
    //output logic [0:Q-1] eout;

    logic [0:P-1] ein;
    logic eout;
    logic [0:P-1][0:2] weights;
    logic [0:P-1] inc;
    logic [0:P-1] dec;
    logic gclk_pulse;


    edge2pulse ep (.pulse_out(gclk_pulse),
                   .edge_in(gclk),
                   .clk_in(aclk)
                  );
    ...
    neuron_rnl_ptt #(P, THRESHOLD) ec (.output_spike(output_spike),
                                           .weights(weights),
 ...
 //                                          .weight_update_en(weight_update_en),
    ...
    pulse2edge pe (.edge_out(eout),
                       .pulse_in(output_spike),
                       .aclk(aclk),
                       .grst(gclk_pulse)
                      );
