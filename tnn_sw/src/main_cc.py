...
parser = argparse.ArgumentParser(description="A multilayer CC Temporal Neural Network")

parser.add_argument('--imgsize',        type=int, default=28,\
                    help='size of input image')

parser.add_argument('--rfsize',         type=int, default=8,\
...
parser.add_argument('--theta',          type=int, default=170,\
                    help='spiking threshold of neuron')

parser.add_argument('--theta2',          type=int, default=40,\
                    help='spiking threshold of neuron')
...
theta2          = args.theta2             # Spiking threshold of neurons (second layer)
...
layer2 = TNNColumnLayer(inputsize=rows, rfsize=rows, stride=rows, nprev=neurons, neurons=num_classes, theta=theta2,\
                        timeres=timeres, wres=wres, ntype=ntype, ramp=ramp, w_init=w_init, k=k, stoch=stoch, reward_en=1)
...
for epochs in range(1):
    ...
    for idx, (data,target) in enumerate(train_loader):
        ...
        weights_out            = layer1.stdp(layer_in, layer_out, layer1.weights.clone(), rvcapture[idx], rvsearch[idx],\
                                          rvbackoff[idx], rvmin[idx], rvstickup[idx], rvstickdown[idx])
        layer1.weights         = weights_out
        endt                   = time.time()
...
ucapture        = 240           # STDP's "capture" Bernoulli random variable probability
usearch         = 16            # STDP's "search" Bernoulli random variable probability
ubackoff        = 768           # STDP's "backoff" Bernoulli random variable probability
...
for epochs in range(1):
    ...
    for idx, (data,target) in enumerate(train_loader):
        ...
        print("Iteration: {0}\r".format(idx), end="")
        data                            = data[0]
        layer_out, layer_in, layer_out1 = layer1(data)
        layer_out2, layer_in2, _        = layer2(layer_out1)
        if torch.nonzero(layer_out2[:,0] != float('Inf')).shape[0] == 1:
            count = count + 1
        arg                             = torch.nonzero(layer_out2[:,0] != float('Inf'))
        if arg.shape[0] == 0:
            reward = 0
        elif arg[0] == target[0]:
            reward = 1
            ...
            reward = -1
        weights_out                     = layer2.stdp(reward, layer_in2, layer_out2,\
                                                      layer2.weights.clone(),rvcapture[idx], rvsearch[idx],\
                                                      rvbackoff[idx], rvmin[idx], rvstickup[idx],\
                                                      rvstickdown[idx])
        layer2.weights                  = weights_out
        endt = time.time()
        print("                                 Time elapsed: {0}\r".format(endt-start), end="")
    
    print("Second Layer Coverage: ", count/(idx+1))
    end = time.time()
    print("Second layer training done in ", end-start)
...
#     save_image(out, '../images/column/visweights.png', nrow=neurons//2)
    ...
    image_list = []
    for i in range(10):
        temp = layer2.weights[i]
        image_list.append(temp.reshape(20,15))
    out = torch.stack(image_list, dim=0).unsqueeze(1)
    save_image(out, '../images/cvt/layer2weights.png', nrow=5)
