...
parser.add_argument('--theta',          type=int, default=27,\
...
layer2 = TNNColumnLayer(inputsize=rows, rfsize=2, stride=1, nprev=neurons, neurons=24, theta=10,\
                        timeres=3, wres=wres, ntype=ntype, ramp=ramp, w_init=w_init, k=k, stoch=stoch, reward_en=0)

layer3 = TNNVoterTallyLayer(rows=25, cols=25, nprev=24, num_classes=num_classes, voter_thresh=voter_thresh,\
...
for epochs in range(1):
    ...
    for idx, (data,target) in enumerate(train_loader):
        ...
        print("Iteration: {0}\r".format(idx), end="")
        data                            = data[0]
        _, _, layer1_out                = layer1(data)
        layer_out, layer_in, _          = layer2(layer1_out)
        weights_out                     = layer2.stdp(layer_in, layer_out, layer2.weights, rvcapture[idx], rvsearch[idx],\
                                                      rvbackoff[idx], rvmin[idx], rvstickup[idx], rvstickdown[idx])
        endt                            = time.time()
        print("                                 Time elapsed: {0}\r".format(endt-start), end="")
    
    end   = time.time()
    print("Second layer training done for {0} in {1} seconds".format(idx, int(end-start)))
...
if weights_save == 1:
    ...
    save_image(out, '../images/ccvt/layer1_visweights.png', nrow=cols)
...
print("Starting third layer training")
for epochs in range(1):
    start = time.time()
    ...
    for idx, (data,target) in enumerate(train_loader):
        ...
        data                            = data[0]
        _, _, layer1_out                = layer1(data)
        layer2_out, _, _                = layer2(layer1_out)
        voter_in, voter_out, prediction = layer3(layer2_out)
        weights_out                     = layer3.stdp(target[0], prediction, voter_in, voter_out,\
                                                      layer3.weights,rvcapture[idx], rvsearch[idx],\
    ...
    print("Third layer training done for {0} in {1} seconds".format(idx, (end-start)))
...
if weights_save == 1:
    ...
    out = torch.stack(image_list, dim=0).unsqueeze(1)
    save_image(out, '../images/ccvt/layer1weights.png', nrow=cols)
    ...
    out = torch.stack(image_list, dim=0).unsqueeze(1)
    save_image(out, '../images/ccvt/layer2weights.png', nrow=cols)
    
    image_list = []
    for i in range(rows*cols):
        temp = layer3.weights[i*neurons:(i+1)*neurons]
        image_list.append(temp)

    out = torch.stack(image_list, dim=0).unsqueeze(1)
    save_image(out, '../images/ccvt/layer3weights.png', nrow=cols)
