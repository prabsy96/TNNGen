...
class Layer():
    ...
    def CV_Layer(self, layer_id=None):
        ...
        input_spikes_dist = m.Input('input_spikes_dist', num_col.value*p_dist.value)
        ...
        # for c in range(num_col.value):
        #     output_spikes.append(m.Output('output_spikes'+'_'+str(c), num_neurons.value))
        output_spikes = m.Output('output_spikes'+'_'+str(c), num_col.value*num_neurons.value)
        ...
        for c in range(num_col.value):
            ...
            # w_init_dist
            col_ports = tnn_func.append_port3(col_ports, w_init_dist, c, num_neurons.value, num_seg.value, p_dist.value)
            # w_init_prox
            col_ports = tnn_func.append_port3(col_ports, w_init_prox, c, num_neurons.value, num_seg.value, p_prox.value)
            # capture_brv_dist
            col_ports = tnn_func.append_port2(col_ports, capture_brv_dist, c, num_neurons.value, num_seg.value)
            # capture_brv_prox
            col_ports = tnn_func.append_port2(col_ports, capture_brv_prox, c, num_neurons.value, num_seg.value)
            # minus_brv_dist
            col_ports = tnn_func.append_port2(col_ports, minus_brv_dist, c, num_neurons.value, num_seg.value)
            # minus_brv_prox
            col_ports = tnn_func.append_port2(col_ports, minus_brv_prox, c, num_neurons.value, num_seg.value)
            # search_brv_dist
            col_ports = tnn_func.append_port2(col_ports, search_brv_dist, c, num_neurons.value, num_seg.value)
            # search_brv_prox
            col_ports = tnn_func.append_port2(col_ports, search_brv_prox, c, num_neurons.value, num_seg.value)
            # backoff_brv_dist
            col_ports = tnn_func.append_port2(col_ports, backoff_brv_dist, c, num_neurons.value, num_seg.value)
            # backoff_brv_prox
            col_ports = tnn_func.append_port2(col_ports, backoff_brv_prox, c, num_neurons.value, num_seg.value)
            # min_brv_dist
            col_ports = tnn_func.append_port2(col_ports, min_brv_dist, c, num_neurons.value, num_seg.value)
            # min_brv_prox
            col_ports = tnn_func.append_port2(col_ports, min_brv_prox, c, num_neurons.value, num_seg.value)
            # F_brv_dist
            col_ports = tnn_func.append_port2(col_ports, F_brv_dist, c, num_neurons.value, num_seg.value)
            # F_brv_prox
            col_ports = tnn_func.append_port2(col_ports, F_brv_prox, c, num_neurons.value, num_seg.value)
