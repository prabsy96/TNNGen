...
class TNN_Functions():
	def Less_equal(self):
		...
		# aclk = ports['aclk']
		# temp1 = ports['pulse_in']
		# rst = ports['grst']
		# temp2 = ports['edge_out']
	...
	def Pulse2edge(self):
		...
		m.Always(Posedge(aclk)) ( If(grst) (temp(Int(0, width = 1, base = 2))) .Else (temp(edge_out)))
	...
	def Wta(self, Q = 10):
		...
		for j in range(q.value): 
			#Submodule(m, less_equal, name = 'l1_'+str(j), arg_ports = [ec_spikes[j], first_spike, aclk, grst, temp[j]])
			lq_inst = m.Instance(less_equal, 'l1_'+str(j), params = None, ports = [ec_spikes[j], first_spike, aclk, grst, temp[j]])
	...
	def Fsm_simple(self):
		...
		#fsm = FSM(m, 'state', aclk, rst)
		#fsm_v = fsm.state
		#fsm.If(in_v)(in_v(1))
		#fsm.If(fsm == fsm(in_v)(fsm.goto_init)
		# Fsm.Else(fsm.goto_next)
		# #fsm.goto_next(in_v==1)
		#fsm.goto_next()
		# fsm.goto_next()
		# fsm.goto_next()
		# fsm.goto_next()
	...
	def Pac(self, ip_size = 32, thres = 13):
		...
		get_cat = Cat(Int(value = 0, width = maxres.value - out_res.value, base = 2), tout)
		...
		m.Instance(adder, 'adder2_in_pac', params = cnt_new, ports = [
			#Int(value = tout.value , width = maxres.value, base = 2),
			get_cat,
			#Slice(tout, maxres.value-1, 0),
			fout, 
			in_v[in_size.value-1], 
			t2out])

		muxout.assign(Cond((out_v | grst) == 1, true_value = Int(-1*thres.value, width = maxres.value, base =2 ),  false_value = Slice(t2out, maxres.value,  1)))
