    ...
    def Wta(self, Q=10):
        ...
        for j in range(q.value):
            lq_inst = m.Instance(less_equal, 'l1_'+str(j), params=None,
                                 ports=[ec_spikes[j], first_spike, aclk, grst, temp[j]])
            li_out[q.value-1].assign(temp[q.value-1])
    
        for k in range(q.value-2, -1, -1):
            li_out[k].assign(temp[k] & ~ Uor(Slice(temp, q.value-1, k+1)))
...
# In[21]:
    ...
    def Flogic(self):
        ...
        m.Always()(If(input_weight == Int(0, width=3, base=2))(out(0))
                   .Elif(input_weight == Int(1, width=3, base=2))(out(F[5]))
                   .Elif(input_weight == Int(2, width=3, base=2))(out(F[4]))
                   .Elif(input_weight == Int(3, width=3, base=2))(out(F[3]))
                   .Elif(input_weight == Int(4, width=3, base=2))(out(F[2]))
                   .Elif(input_weight == Int(5, width=3, base=2))(out(F[1]))
                   .Elif(input_weight == Int(6, width=3, base=2))(out(F[0]))
...
# In[22]:
    ...
    def Stdp_case_gen(self):
        ...
        pulse_inst = m.Instance(pulse, 'pe', params=None,  ports=[
                                aclk, temp, grst, greater])
    
        tboth.assign(ein & eout)
        tone.assign(ein ^ eout)
    
        stdp_cases[3].assign(~ greater & tboth)
        stdp_cases[2].assign(greater & tboth)
        stdp_cases[1].assign(~ greater & tone)
        stdp_cases[0].assign(greater & tone)
...
# In[23]:
    ...
    # eight-cycle wide pulse generator
    def Fsm_simple(self):
        ...
        temp.assign(~ state[2] | state[1] | state[0])
        ...
        m.Always(Posedge(aclk))(
            ...
            .Else(
                If(gclk)(
                    dout(0),
                    If(state == S0)(
                        If(tinc & weight_update_en == 1)(
                            state(S1)
                        )
    
                    )
                    .Elif(state == S1)(
                        If(tdec & weight_update_en == 1)(
                            state(S0)
                        )
                        .Elif(tinc & weight_update_en == 1)(
                            state(S2)
                        )
                    )
...
# In[25]:
    ...
    def Stdp(self):
        ...
        stdp_case_gen_inst = m.Instance(stdp_case, 's1', params=None, ports=[
                                        ein, eout, aclk, grst, cases])
        flogic_inst = m.Instance(flogic, 's2', params=None,
                                 ports=[F, input_weight, fout])
        incdec_inst = m.Instance(incdec, 's3', params=None, ports=[
                                 cases, capture, minus, search, backoff, min_v, fout, inc, dec])
...
# In[26]:
    ...
    def Pac(self, ip_size=32, thres=13):
        ...
        get_cat = Cat(Int(value=0, width=maxres.value -
                      out_res.value, base=2), tout)
    
        for i in range(stages.value):
    
            j = Div(in_size.value, Sll(1, (i+2)))
            for j in range(i+1):
                m.Instance(adder, 'a1_'+str(i)+str(j), params=count, ports=[
                    Slice(temp, Srl(in_size.value, i)*((Sll(1, i+1))-i-2) + 2*j *
                          (i+1)+i, Srl(in_size.value, i)*((Sll(1, i+1))-i-2) + 2*j*(i+1)),
                    Slice(temp, Srl(in_size.value, i)*((Sll(1, i+1))-i-2) + (2*j+1) *
                          (i+1)+i, Srl(in_size.value, i)*((Sll(1, i+1))-i-2) + (2*j+1)*(i+1)),
                    in_v[Add(in_size_val, (Srl(in_size.value, i+1))*((Sll(1, i)-1))+j)],
                    Slice(temp, Srl(in_size.value, i+1)*(Sll(1, i+2)-i-3) + j*(i+2) +
                          (i+1), Srl(in_size.value, i+1)*(Sll(1, i+2)-i-3) + j*(i+2))
                ])
    
            count[0] = i+1
    
        cnt_new = []
        cnt_new.append(maxres.value)
    
        c = []
        c.append('MAXRES\'(tout)')
    
        tout.assign(Slice(temp, num.value-1, num.value - out_res.value))
    
        m.Instance(adder, 'adder2_in_pac', params=cnt_new, ports=[
            get_cat,
            fout,
            in_v[in_size.value-1],
            t2out])
    
        muxout.assign(Cond((out_v | grst) == 1, true_value=Int(-1*thres.value,
                      width=maxres.value, base=2),  false_value=Slice(t2out, maxres.value,  1)))
...
# In[27]:
    ...
    def Neuronbody(self, ip_size=16, thres=13):
        ...
        pac_inst = m.Instance(pac, 'p1', params=par_pac, ports=[
                              acc_in, aclk, pac_rst, temp])
    
        fsm_simple_inst = m.Instance(fsm_s, 'fs', params=None, ports=[
                                     aclk, rst, temp, out_v])
...
# In[28]:
    ...
    def NeuronRNL(self, ip_size=16, thres=13):
        ...
        nbody, nbody_clk = self.Neuronbody(
            ip_size=in_size.value, thres=thres.value)
    
        for i in range(in_size.value):
            m.Instance(fsm_s, 'f1_'+str(i), params=None,
                       ports=[weight_en, aclk, gclk, rst, in_v[i], inc[i], dec[i], up_in[i], weight[i]])
